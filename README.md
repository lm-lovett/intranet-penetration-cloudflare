...
Worker 只做发现和中继。两台电脑都加入同一个网络后，可以互访虚拟 IP；要访问公司局域网里的其它设备，必须在**公司那台已经能上内网的电脑**上打开子网代理。

1. 两台都用 easytier-gui（2.6.4+），配置与第 2 节相同：手动、`wss://lm191549149.me:443/`、同一网络名和密码、DHCP、安全模式。每台自己生成一套 `[secure_mode]` 密钥，不要两台抄同一对。
2. **公司电脑**（须一直开着、EasyTier 保持运行）：**子网代理CIDR** 填公司内网，例如 `192.168.10.0/24`。在 `ipconfig` / `ip addr` 里看实际网段，不要照抄。不要勾「启用出口节点」，除非你想让家里所有上网都走公司。
3. **家里电脑**：不要填公司那个网段的子网代理。运行后节点列表里应看到公司机器。
4. 访问公司这台电脑：用它的 **虚拟IPv4**（远程桌面、SSH、共享文件夹）。访问公司里其它设备：用它们在公司内网的 IP。
5. 若家里和公司都是 `192.168.1.0/24`，会冲突。公司子网代理改成映射，例如 `192.168.1.0/24->10.33.1.0/24`，家里访问 `10.33.1.x` 等于公司 `192.168.1.x`。
6. 内网网站（例如解析到 `172.17.x.x` 的域名）不用改 URL。公司电脑能打开该网站时，把这个 IP 所在网段也填进公司节点的子网代理，家里用原域名访问即可。

Windows 需要管理员权限才能建 TUN。公司电脑防火墙放行 EasyTier。只在你有权访问该内网时使用。
